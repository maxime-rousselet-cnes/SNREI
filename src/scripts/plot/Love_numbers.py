...
def plot_Love_numbers_for_options_for_descriptions_per_type(
    ...
    In this grid, every column correspond to a real description (thus, arheological model).
    ...
    anelastic_results: dict[tuple[str, bool, bool], Result] = {
        (real_description_id, option.use_long_term_anelasticity, option.use_short_term_anelasticity): Result()
        for real_description_id, option in product(real_description_ids, options)
    }
    ...
    for real_description_id in real_description_ids:
        for option in options:
            result_subpath = results_path.joinpath(
                get_run_folder_name(real_description_id=real_description_id, run_id=option.run_id())
            )
            anelastic_results[real_description_id, option.use_long_term_anelasticity, option.use_short_term_anelasticity].load(
                name="anelastic_Love_numbers", path=result_subpath
            )
            T_values[real_description_id, option.use_long_term_anelasticity, option.use_short_term_anelasticity] = (
                frequencies_to_periods(frequencies=load_base_model(name="frequencies", path=result_subpath))
            )
        elastic_results[real_description_id].load(name="elastic_Love_numbers", path=result_subpath.parent.parent)
    ...
    for direction in directions:
        for boundary_condition in boundary_conditions:
            ...
            for zoom_in in BOOLEANS:
                ...
                _, plots = plt.subplots(2, len(real_description_ids), figsize=(16, 9), sharex=True)
                for part in ["real", "imaginary"]:
                    for real_description_id, plot in zip(real_description_ids, plots[0 if part == "real" else 1]):
                        ...
                        for i_degree, degree in zip(degrees_indices, degrees_to_plot):
                            for option in options:
                                ...
                                complex_result_values = anelastic_results[
                                    real_description_id, option.use_long_term_anelasticity, option.use_short_term_anelasticity
...
def plot_Love_numbers_for_options_per_description_per_type(
    ...
    for real_description_id in real_description_ids:
        plot_Love_numbers_for_options_for_descriptions_per_type(
            ...
            figure_subpath_string=real_description_id + "/Love_numbers",
